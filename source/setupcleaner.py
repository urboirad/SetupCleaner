...
def get_assets_folder():
    if hasattr(sys, '_MEIPASS'):
        ...
        return os.path.join(os.path.dirname(os.path.abspath(__name__)), 'assets')

# Directory Setup
dir_path = os.path.dirname(os.path.abspath(__name__))
true_path = getattr(sys, '_MEIPASS', os.path.dirname(os.path.abspath(__name__)))
assets_folder = get_assets_folder()

font_name_ext = 'VCRosdNEUE.ttf'
font_name = 'VCRosdNEUE'
font_path = os.path.join(assets_folder, font_name_ext)
font.add_file(font_path)
VCRosdNEUE = font.load(font_name, 16)

img = 'logo.png'
img_path = os.path.join(assets_folder, img)
logo = image.load(img_path)
